...
impl SourceMap {
    ...
    pub fn load(&mut self, wasm_bytes: &[u8]) -> Result<()> {
        let obj = object::File::parse(wasm_bytes)
            .context("Failed to parse WASM object file")?;
        ...
        let load_section = |id: gimli::SectionId| -> Result<EndianSlice<RunTimeEndian>, gimli::Error> {
            let section = obj.section_by_name(id.name()).unwrap_or_else(|| {
                obj.section_by_name(&format!(".{}", id.name())).unwrap_or_else(|| {
                    object::Section {
            ...
            Ok(EndianSlice::new(section.data().unwrap_or(&[]), RunTimeEndian::Little))
        };
        ...
        while let Some(header) = units.next()? {
            ...
            if let Some(program) = unit.line_program.clone() {
                ...
                while let Some((header, row)) = rows.next_row()? {
                    if let Some(file_path) = self.get_file_path(&dwarf, &unit, header, row.file_index()) {
                        ...
                        self.offsets.insert(offset, SourceLocation {
                            file: file_path,
                            line,
                            column,
                        });
    ...
    pub fn get_source_line(&mut self, location: &SourceLocation) -> Option<String> {
        ...
        content.lines().nth(location.line.saturating_sub(1) as usize).map(|s| s.to_string())
    }
